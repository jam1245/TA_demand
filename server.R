...
#      })
    ...
    output$forecast_table = DT::renderDataTable({
      ...
      DT::datatable(forecast_grps %>% filter(forecast_group == input$forecast_grp), options = list(lengthMenu = c(20, 30, 50), pageLength = 20))
 ...
 #     forecast_grps %>% #select(-X)
  #      filter(forecast_group == input$forecast_grp) %>% tail(14)
    ...
    output$mytable3 <- DT::renderDataTable({
      ...
      table <- forecast_grps %>% filter(forecast_group == input$forecast_grp) %>% lrp_data_transform_viz()
      DT::datatable(table, options = list(lengthMenu = c(20, 30, 50), pageLength = 12))
    })
    
    
    output$lrp_plotly_range <- renderPlotly({
      plot_lrp_forecast(forecast_grps, parameter = input$forecast_grp)
    })
